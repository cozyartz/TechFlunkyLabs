...
export default function WhyCloudflare() {
  return (
    <section id="cloudflare" className="py-24 px-6 bg-surface-900/30">
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <h2 className="text-4xl md:text-5xl font-bold mb-4">
            Why We Build on <span className="text-orange-400">Cloudflare</span>
          </h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          {benefits.map((benefit, index) => (
            <motion.div
              ...
              className="p-5 rounded-xl bg-surface-900/50 border border-surface-800"
            >
              <benefit.icon className="w-6 h-6 text-orange-400 mb-3" />
              <h3 className="font-semibold text-surface-100 mb-2">{benefit.title}</h3>
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          transition={{ duration: 0.6 }}
          className="p-8 rounded-2xl bg-gradient-to-br from-orange-500/10 to-primary-500/10 border border-orange-500/20"
        >
          <h3 className="text-2xl font-semibold mb-6 text-center text-surface-100">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {cloudflareProducts.map((product) => (
              <div
                key={product.name}
                className="p-3 rounded-lg bg-surface-900/50 border border-surface-800"
              >
                <div className="font-medium text-surface-100">{product.name}</div>
                <div className="text-xs text-surface-500">{product.desc}</div>
              </div>
            ))}
          </div>
        </motion.div>
